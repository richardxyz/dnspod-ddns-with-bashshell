...
URLIP=$(wget -q -O - $CHECKURL|grep -Eo "$IPREX"|tail -n1)
...
Record="$(wget -q -O - --no-check-certificate --post-data="${token}"  https://dnsapi.cn/Record.List)"
...
ddns="$(wget -q -O - --no-check-certificate --post-data="${token}&record_id=${record_id}&record_line_id=${record_line_id}"  https://dnsapi.cn/Record.Ddns)"
